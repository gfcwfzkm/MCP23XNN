/** MCP23x08/MCP23x17 Library
 *
 * C Library for the MCPx08 and MCPx17 Port expanders, supporting both the I2C and SPI interface
 *
 * Created: 10.01.2021
 *  Author: gfcwfzkm
 */
...
typedef struct {
	...
	uint8_t (*transceiveBytes)(void*,	// Send/Get data function pointer: InterfacePointer,
						uint8_t,		// Address of the PortExpander (8-Bit Address Format!),
						uint8_t*,		// Pointer to receive buffer,
						uint16_t);		// Amount of bytes to receive
...
/* @brief Creating an SPI structure
 *
 * Requires the basic I/O driver functions to be passed, as well as the interface
 * needed to use the I/O drivers. To use the driver for a SPI-variant of the chip,
 * only a transceiveBytes-function is required
 *
...
/* @brief Creating an I2C structure
 *
 * Requires the basic I/O driver functions to be passed, as well as the interface
 * needed to use the I/O drivers.To use the driver for a I2C-variant of the chip,
 * only a sendData and getData function is required
 *
 * @param inst				pointer to the mcp23x_t struct
 * @param mcp_type			Choose MCP23X08 or MCP23X17
 * @param mcp_subAddr		Pass over the Sub-Address, selectable by the A0-A2 pins
 ...
 *
 * @param inst				Pointer to the configured mcp23x_t struct
 * @param _prt				Either MCP23_PORTA or MCP23_PORTB (PORTB only on MCP23X17)
